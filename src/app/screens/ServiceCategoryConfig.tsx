  ...
  return (
    <AdminLayout>
      ...
      <div className="mb-8">
        ...
        <h1 className="text-[24px] tracking-tight text-black dark:text-white/95 mb-1">Service Categories</h1>
        <p className="text-[14px] text-black/60 dark:text-white/60">Organize services into categories.</p>
      </div>
      <div className="flex items-start justify-between mb-6">
        <div className="bg-white/85 dark:bg-white/[0.02] backdrop-blur-[40px] border border-black/5 dark:border-white/5 rounded-[24px] p-6 min-w-[200px]">
          <div className="text-[10px] font-bold tracking-[0.05em] text-black/40 dark:text-white/40 uppercase mb-2">CATEGORIES</div>
          <div className="text-[32px] font-bold text-black dark:text-white/95 mb-1">{serviceCategories.length}</div>
        </div>
        <button onClick={() => handleOpenModal()} className="px-6 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-[12px] text-[14px] font-medium transition-all flex items-center gap-2 shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredCategories.map((category) => (
          <div key={category.id} className="bg-white/85 dark:bg-white/[0.02] backdrop-blur-[40px] border border-black/5 dark:border-white/5 rounded-[24px] p-6">
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-start gap-3 flex-1">
                <div className="p-2 bg-blue-500/10 rounded-[12px]">
                  <Layers className="w-5 h-5 text-blue-500" />
                </div>
                <div className="flex-1">
                  <h3 className="text-[16px] font-medium text-black dark:text-white/95 mb-1">{category.name}</h3>
                  <p className="text-[14px] text-black/60 dark:text-white/60">{category.description}</p>
                </div>
              </div>
              <div className="flex items-center gap-1">
                <button onClick={() => handleOpenModal(category)} className="p-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg transition-colors">
                  <Edit2 className="w-4 h-4 text-black/40 dark:text-white/40" />
                </button>
                <button onClick={() => handleDelete(category.id)} className="p-2 hover:bg-red-500/10 rounded-lg transition-colors group">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-900 rounded-[24px] p-8 w-full max-w-md shadow-2xl border border-black/10 dark:border-white/10">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-[20px] font-semibold text-black dark:text-white">{editingCategory ? 'Edit Category' : 'Add Category'}</h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                  className="w-full px-4 py-2.5 bg-white dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-[12px] text-[14px] text-black dark:text-white focus:outline-none focus:border-emerald-500/50" placeholder="e.g., Legal Services" />
            ...
            <div className="flex items-center gap-3 mt-6">
              ...
              <button onClick={handleSubmit} className="flex-1 px-4 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-[12px] text-[14px] font-medium transition-all shadow-sm">{editingCategory ? 'Update' : 'Create'}</button>
            </div>
          </div>
        </div>
      )}
